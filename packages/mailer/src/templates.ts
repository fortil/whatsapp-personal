/**
 * Plantillas de correo de la fase de auth. Email-safe a conciencia: tablas
 * anidadas con role="presentation", CSS 100% inline, nada de flex ni grid
 * (los clientes de correo los ignoran o los rompen).
 */
...
/** Escape mínimo de HTML para los datos de usuario que se interpolan. */
...
function codeRow(code: string): string {
  const cells = code
    ...
    .map(
      (d) =>
        `<td align="center" style="padding:0 4px;"><div style="width:44px;height:52px;line-height:52px;border:1px solid #D9E4DC;border-radius:10px;background:#FFFFFF;font-size:24px;font-weight:700;color:${TEXT};font-family:'Instrument Sans',Arial,sans-serif;text-align:center;">${d}</div></td>`,
    )
    .join('')
...
<table role="presentation" border="0" cellpadding="0" cellspacing="0" width="100%" style="background:${BG};padding:24px 12px;">
<tr><td align="center">
<table role="presentation" border="0" cellpadding="0" cellspacing="0" width="480" style="max-width:480px;width:100%;background:#FFFFFF;border:1px solid #D9E4DC;border-radius:14px;overflow:hidden;">
...
<tr><td style="padding:8px 32px 0 32px;font-family:'Bricolage Grotesque',Arial,sans-serif;font-size:22px;font-weight:700;color:${TEXT};">${title}</td></tr>
...
<tr><td style="padding:0 32px 24px 32px;font-family:'Instrument Sans',Arial,sans-serif;font-size:12px;line-height:18px;color:${MUTED};border-top:1px solid #D9E4DC;">Si no esperabas este correo, ignóralo. No reenvíes códigos a nadie.</td></tr>
</table>
</td></tr>
</table>
</body>
...
export function accountApprovedEmail(): MailContent {
  return {
    subject: `${BRAND}: tu cuenta fue aprobada`,
    text: 'Tu cuenta fue aprobada. Ya puedes iniciar sesión.',
    html: layout(
      'Tu cuenta fue aprobada',
      `<p style="margin:0;">William aprobó tu registro. Ya puedes <a href="#" style="color:${ACCENT};font-weight:700;">iniciar sesión</a> y vincular tu WhatsApp.</p>`,
    ),
  }
}
  ...
  return {
    ...
    html: layout(
      ...
      `<p style="margin:0 0 4px 0;">Un administrador restableció tu contraseña. Tu acceso temporal:</p>` +
        `<div style="margin:16px 0;padding:12px 16px;border:1px solid #D9E4DC;border-radius:10px;background:#FFFFFF;font-family:'Instrument Sans',Arial,sans-serif;font-size:18px;font-weight:700;color:${TEXT};text-align:center;">${password}</div>` +
